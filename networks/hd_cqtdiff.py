...
class ResnetBlock(nn.Module):
    def __init__(
        ...
        for i in range(self.num_dils):
            ...
            self.H.append(Conv2d(N, N, kernel=kernel_size, dilation=(2**i, 1), bias=bias, **init))  # freq convolution (dilated)
...
class CombinedTFR(nn.Module):
    """
    Classe com a implementação da combinação de representações. 
    Tem que ter um método __init__ e um método forward
    """
    pass

class HD_CQT_Diff(nn.Module):
    """
    CQTDiff+ with combined TFRs added to it
    """
    ...
    def __init__(self, args, device):
        ...
        """
        super(HD_CQT_Diff, self).__init__()
        self.args = args
        ...
        self.emb_dim = args.network.emb_dim # dimensao do embedding de ruído
        ...
        self.CQTransform = CQT_nsgt(
            self.args.network.cqt.num_octs,
            self.args.network.cqt.bins_per_oct,
            mode="oct",
            window=self.win,
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        ...
                        Conv2d(2, dim_out, kernel=(5, 3), bias=False, **init), # skip connection
                        ResnetBlock( # Res. Block
                            dim_in,
                            dim_out,
                            self.use_norm,
    ...
    def forward(self, inputs, sigma):
        ...
        # Aqui certamente vamos ter que mudar fazendo alguma função / remendo
        # Provavelmente algo que concatene as oitavas que nós desejamos pro primeiro nível
        X_list = self.CQTransform.fwd(inputs.unsqueeze(1))
        X_list_out = X_list

        # Aqui deve entrar a chamada pro módulo da representação combinada.
        # Se for uma combinação por nível, vai ser necessário fazer uma lista como X_list

        # X_list_out vai receber as oitavas reconstruídas.
        # Tem que ser uma oitava por índice da lista, pra não quebrar a volta da CQT por oitava do Moliner
        ...
        for i, modules in enumerate(self.downs):
            if i <= (self.num_octs - 1):
                ...
                # init block --> In. Block
                # pyr_down_proj --> skip connection (Conv 1x1)
        ...
        for i, modules in enumerate(self.ups):
            ...
            if j <= (self.num_octs - 1):
                ...
                # Aqui ele vai jogando fora as features das oitavas que ele já reconstruiu
                # Vamos ter que olhar com cuidado aqui na hora da reconstrução
        ...
        # O método bwd do módulo CQTransform desfaz a CQT de maneira diferenciável
        # No entanto ele precisa das oitavas separadas em uma lista, com a resolução temporal aumentando
